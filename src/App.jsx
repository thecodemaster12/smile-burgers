...
const App = () => {
  ...
  const [currentSelectedItem, setCurrentSelectedItem] = useState(burgerMenu[0].items[0])
      ...
      updatedItems = prev.items.map((cartItem) =>
        cartItem.id === item.id
          ? {
              ...
              totalPrice:
                (cartItem.quantity + cart) * cartItem.price,
            }
          : cartItem
      );
    } else {
  ...
  return (
    <>
      <div className="min-h-screen bg-gray-100 flex flex-col items-center relative">
        ...
        <div className={`fixed top-0 left-0 w-3/5 h-full bg-white shadow ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}>
        ...
        <div className="py-4">
          <h2 className="text-center text-3xl font-bold font-pt-sans py-2">Choose Your Favorite</h2>
          <div className="flex gap-2">
            <span className="badge">Burgers</span>
            <span className="badge">Sand witch</span>
            <span className="badge">Pizzas</span>
            <span className="badge">Drinks</span>
          </div>
        </div>
        ...
        <div className="menu-wrapper flex max-w-4xl w-full min-h-96">
          <div className="menu-left flex-1 p-3 rounded bg-primary">
            ...
            {burgerMenus.map((category) => (
              <div key={category.categoryName} className="menu-items border-t mt-6 first:mt-0">
                <h4 className="item-heading uppercase text-2xl font-bold mb-1">
                  {category.categoryName}
                </h4>

                {category.items.map((item) => (
                  <button key={item.id} onClick={() => updateCurrentSelectedItem(item)} className="menu-item w-full text-left flex items-end hover:bg-orange-300 transition-all duration-150 px-2 py-1 rounded">
                    <span>{item.name}</span>
            ...
            <div className="menu-card space-y-4">
              ...
              <div className="menu-card-content space-y-4">
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <span className="time-to-make badge">{currentSelectedItem.timeToMake}</span>
                  <span className="calories badge">{currentSelectedItem.calories} cal</span>
                </div>
                ...
                <div className="order-area flex gap-4">
                  ...
                  <button onClick={() => handelAddToCard(currentSelectedItem)} className="add-to-cart bg-primary hover:bg-orange-400 transition-all duration-150 flex-1 rounded text-white text-xl font-bold">
                    Add
                  </button>
                </div>
              </div>
